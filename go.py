...
class gas:
        ...
        def get_model(self):
            ...
            try:
                self.dot =json.dumps(model['item']['upcoming_flash_sale']['promotionid'])
                self.get =json.dumps(model['item']['upcoming_flash_sale']['modelids'][0])
            except TypeError:
        ...
        def add(self):
            ...
            if self.addon == 'null':
                ...
                "modelid":int(idm),
                "promotionid":int(self.dot) #dapet dari split product
                }
            else:
                    payload1 = {
                "quantity":1,
                ...
                "source":"{\"refer_urls\":[]}",
                "client_source":1,
                "add_on_deal_id":int(idm),
                "shopid":int(ids), #dapet dari item model
                "itemid":int(idi), #dapet dari split product
                # "modelid":int(idm) #dapet dari split product
                }
            while 1:
